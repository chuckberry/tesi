...
worst-case execution time (WCET) denoted by $C(T)$. It means that each job released by task $T$ has a maximum duration of $C(T)$. For each job is defined 
...
indentifies the task and indicates the priority of the latter. Higher priority is 1 and lower priority is 4. In the picture jobs are represented in this 
way: $J_{\alpha}^\beta$, where $\alpha$ represents which task has released the job, and then also its priority, and $\beta$ is an index that identifies 
the job. We assume that co-scheduled jobs use not-overlapped cache partitions.
...
jobs to infer if cache thrasing occurs. Nevertheless, in practicei, this is not always true, because it greatly depends on type of application executed.
...
of each scheduled task and provide these informations to scheduler. The challenge is how to do this job. 
...
increases priority, of the job with the smallest WSS, in this way jobs on a runqueue are ordered by WSS.
...
of the queue and it will be the next task to execute. If there are tardy jobs, they are inserted in EDF-ordered queue and they are scheduled according to 
...
thrashing will occur. In this case, the core remains idle and the job waits for some job that free cache resources.
...
and multiplying the result by the cache line size. Cache misses and WSS are related, because, from experiments executed by Calandrino et al, follows that
number of cache misses multiplied for dimension of a cache line gives a number proportional to WSS used by a job. All the profiled jobs belong to the same 
task. The co-scheduler discards measures obtained by jobs that cause cache thrashing. Cache misses are recorded using performance counters. 
At the beginning of an application, there is not any measure about cache miss, therefore the profiler requires a bootstrapping phase in order to get 
necessary measures. This phase converge pretty fast to a reasonable result.
...
The reason is simple: the WSSs of two jobs could be overlapped. It is very difficult in practice having not-overlapping WSS. Furthermore, also 
...
Nevertheless, the policy improves predictability of the application, because, theoretically, each job should find in cache enough space for its resources, 
mantaining constant its execution time. This aspect make this policy very indicate for Real-time system, where predictability is a very important aspect.
